...
const AdminDashboard = () => {
    ...
    const loadAdmin = async () => {
        try {
            const response = await axios.get(`http://64.23.186.50/api/workers/${adminid}`);
    ...
    const handleUpdatePassword = async () => {
        try {
            ...
            await axios.put(`http://64.23.186.50/api/workers/passwordUpdate/${adminid}`, send);
    ...
    const handleConfirmDelete = async (id) => {
        try {
            await axios.delete(`http://64.23.186.50/api/workers/${id}`);
            loadWorkers();
